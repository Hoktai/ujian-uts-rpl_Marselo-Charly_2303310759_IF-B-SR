...
<div class="mb-6 bg-white p-4 rounded-lg shadow flex flex-col sm:flex-row sm:items-center gap-4">
  <form action="{{ route('produk.index') }}" method="GET" class="flex flex-col sm:flex-row sm:items-center gap-4 w-full">
    ...
    {{-- Search Nama Produk --}}
    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama produk..." class="w-full sm:w-1/3 p-2 border rounded text-sm sm:text-base">

    {{-- Filter Kategori --}}
    <select name="kategori" class="w-full sm:w-1/4 p-2 border rounded text-sm sm:text-base">
    ...
    {{-- Tombol Search --}}
    <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition text-sm sm:text-base">
      Cari
    </button>

    {{-- Tombol Reset --}}
    <a href="{{ route('produk.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 transition text-sm sm:text-base">
      Tampilkan Semua
    </a>
  </form>
</div>
